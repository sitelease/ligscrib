export function html(name : string, icons : Map<string, string>, prefix : string, faCompatibility ?: boolean, faUtility ?: boolean) {
    return `<!DOCTYPE html>
<html>
	<head>
		<title>${name}</title>
	...
	${faCompatibility ? `<h1>${name} (Font Awesome Compatible)</h1>` : `<h1>${name}</h1>`}
